...
for i in range(0,int((t_secs[-1]-t_secs[0]-t_int+overlap)/overlap)):
    ...
    Bmag_sitv.append(B_mag[np.argmax(t_secs>subintervals[i][0]):np.argmax(t_secs>subintervals[i][1])])
...
b=[]
bsitv=[]
bsitv_mean = np.empty(int((t_secs[-1]-t_secs[0]-t_int+overlap)/overlap))
peakness= np.empty(int((t_secs[-1]-t_secs[0]-t_int+overlap)/overlap))   
...
for i in range(0,int((t_secs[-1]-t_secs[0]-t_int+overlap)/overlap)):
    bsitv.append(b[np.argmax(t_secs>subintervals[i][0]):np.argmax(t_secs>subintervals[i][1])])
    bsitv_mean[i]=np.mean(bsitv[i])
    peakness[i]=np.mean((bsitv[i]-bsitv_mean[i])**3)/(np.mean((bsitv[i]-bsitv_mean[i])**2))**1.50
    
fig6=plt.figure()
plt.plot_date(timeav,peakness,fmt='-',linewidth=1.0)
plt.xlabel("Time")
plt.ylabel("Peakness")
